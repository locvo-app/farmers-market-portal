  ...
  const handleGoogleSignIn = async () => {
    ...
    try { 
      ...
      if (err.code === 'auth/unauthorized-domain') {
        setErrorMsg("Tên miền này chưa được cấp phép. Vui lòng thêm 'usercontent.goog' vào Authorized Domains trong Firebase Console.");
      } else {
    ...
    return (
      <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white p-12 rounded-[3.5rem] shadow-2xl text-center border border-emerald-50">
           ...
           <button onClick={() => setView('admin_dashboard')} className="text-emerald-700 font-bold text-[10px] uppercase tracking-widest hover:underline mt-4">Quản trị nội bộ MD</button>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-8 animate-in fade-in">
            <div className="bg-[#1B4332] p-12 rounded-[3.5rem] text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 text-center md:text-left">
                ...
                <p className="text-emerald-200">Chào mừng bạn quay lại hệ thống Farmers Market Portal.</p>
              </div>
            ...
            <div className="bg-white rounded-[3rem] shadow-xl border border-emerald-50 overflow-hidden">
              <div className="px-10 py-6 border-b border-emerald-50 bg-[#FDFBF7] font-black text-emerald-800 uppercase text-xs tracking-widest flex items-center gap-2">
                <History size={16}/> Lịch sử nộp hồ sơ của bạn
              </div>
              <div className="divide-y divide-emerald-50">
                {submissions.filter(s => s.userId === user?.uid).length === 0 ? (
                  <div className="p-24 text-center text-gray-300 font-bold italic uppercase tracking-widest text-[10px]">Chưa có dữ liệu chào hàng</div>
                ) : (
                    ...
                    <div key={sub.id} className="p-8 flex justify-between items-center hover:bg-emerald-50/10 transition-all group">
                      <div className="space-y-2">
                        ...
                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                           Mã: #{sub.id.slice(-6).toUpperCase()} • Gửi: {sub.createdAt ? new Date(sub.createdAt.seconds * 1000).toLocaleDateString('vi-VN') : 'Mới'}
                        </p>
                      </div>
             ...
             <form className="max-w-4xl mx-auto space-y-12" onSubmit={e => e.preventDefault()}>
                  ...
                  <div className="space-y-8 animate-in fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-1">
                        ...
                        <input type="text" placeholder="Tên Công ty (Theo GPKD)" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold transition-all shadow-inner" value={formData.companyName} onChange={e => setFormData({...formData, companyName: e.target.value})} />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-black text-gray-400 uppercase ml-1">Mã số thuế *</label>
                        <input type="text" placeholder="MST" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold transition-all shadow-inner" value={formData.taxId} onChange={e => setFormData({...formData, taxId: e.target.value})} />
                      </div>
                    </div>
                  </div>
                )}
                ...
                {step === 2 && (
                  <div className="space-y-8 animate-in fade-in text-center py-10">
                    <FileText size={64} className="text-emerald-600 mx-auto mb-4" />
                    <h3 className="text-2xl font-black text-[#1B4332] uppercase italic">Hồ sơ & Pháp lý</h3>
                    <p className="text-sm text-gray-400">Vui lòng chuẩn bị GPKD và chứng nhận ATTP bản Scan.</p>
                  </div>
                )}
                ...
                {step === 4 && (
                  <div className="space-y-8 animate-in fade-in">
                    <div className="flex justify-between items-center border-b border-emerald-50 pb-6">
                      ...
                      <button type="button" onClick={addProduct} className="bg-[#1B4332] text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-emerald-800 shadow-xl transition-all"><Plus size={18}/> Thêm hàng</button>
                    </div>
                        ...
                        <div key={p.id} className="p-8 border-2 border-emerald-50 rounded-[3rem] bg-white relative hover:shadow-lg transition-all">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div className="md:col-span-2 space-y-1">
                               ...
                               <input type="text" placeholder="Tên sản phẩm" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold text-sm shadow-inner" value={p.name} onChange={e => updateProduct(p.id, 'name', e.target.value)} />
                            </div>
                            <div className="space-y-1">
                               <label className="text-[10px] font-black text-gray-400 uppercase">Giá vốn *</label>
                               <input type="number" placeholder="VNĐ" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold text-sm shadow-inner" value={p.costPrice} onChange={e => updateProduct(p.id, 'costPrice', e.target.value)} />
                            </div>
                            <div className="space-y-1">
                               <label className="text-[10px] font-black text-gray-400 uppercase">Giá bán lẻ dự kiến *</label>
                               <input type="number" placeholder="VNĐ" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold text-sm shadow-inner" value={p.sellingPrice} onChange={e => updateProduct(p.id, 'sellingPrice', e.target.value)} />
                            </div>
                ...
                {step === 5 && (
                  <div className="text-center py-20 space-y-10 animate-in zoom-in-95">
                    ...
                    <h3 className="text-4xl font-black text-[#1B4332] tracking-tight uppercase italic leading-none">Xác nhận cam kết<br/><span className="text-sm opacity-50 uppercase tracking-widest">FARMERS MARKET</span></h3>
                    <p className="text-sm text-gray-400 font-medium leading-relaxed max-w-md mx-auto italic">"Chúng tôi cam đoan các thông tin cung cấp là hoàn toàn trung thực và chịu trách nhiệm trước pháp luật về hồ sơ kỹ thuật sản phẩm."</p>
                  </div>
                )}
