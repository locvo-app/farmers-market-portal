...
import { 
  ...
  User, Briefcase, Phone, Mail, CheckCircle, Settings, ShieldAlert, Store
} from 'lucide-react';
...
const App = () => {
  ...
  const submitRegistration = async () => {
    ...
    if (!formData.companyName || !formData.registrantName || !formData.registrantPhone) {
        setErrorMsg("Vui lòng nhập đủ thông tin Công ty và Người nộp tại Bước 1.");
        setStep(1);
        return;
    }
    ...
    try {
      ...
      setErrorMsg("Lỗi khi gửi hồ sơ lên hệ thống.");
      setLoading(false);
    }
  };
      ...
      <div className="max-w-xl w-full bg-white p-16 rounded-[4rem] shadow-2xl space-y-8 border border-emerald-50 text-center">
        ...
        <div className="space-y-4">
          ...
          <p className="text-gray-500 font-medium text-lg leading-relaxed italic">
            ...
            Bộ phận MD sẽ kiểm tra và phản hồi trực tiếp qua email/SĐT nếu sản phẩm phù hợp.
          </p>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] flex flex-col font-sans antialiased text-left">
              ...
              <div className="space-y-8 animate-in fade-in">
                <div className="bg-[#1B4332] p-12 rounded-[3.5rem] text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="relative z-10 text-center md:text-left">
                    ...
                    <p className="text-emerald-200">Bắt đầu nộp hồ sơ sản phẩm mới và theo dõi thẩm định.</p>
                  </div>
                ...
                <div className="bg-white rounded-[3rem] shadow-xl border border-emerald-50 overflow-hidden">
                  <div className="px-10 py-6 border-b border-emerald-50 bg-[#FDFBF7] font-black text-emerald-800 uppercase text-[10px] tracking-widest">Hồ sơ đã nộp của bạn</div>
                  <div className="divide-y divide-emerald-50">
                    {submissions.filter(s => s.userId === user?.uid).length === 0 ? (
                      ...
                      submissions.filter(s => s.userId === user?.uid).map(sub => (
                        <div key={sub.id} className="p-8 flex justify-between items-center group hover:bg-emerald-50/5">
                          ...
                          <button className="w-14 h-14 bg-[#F0F5F2] text-emerald-600 rounded-2xl flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all"><Eye size={24}/></button>
                        </div>
                      ))
                    )}
                  </div>
                        ...
                        <div className="space-y-8">
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                             <div className="space-y-1">
                               ...
                               <input type="text" placeholder="Công Ty TNHH..." className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold outline-none shadow-inner" value={formData.companyName} onChange={e => setFormData({...formData, companyName: e.target.value})} />
                             </div>
                             <div className="space-y-1">
                               <label className="text-[10px] font-black text-gray-400 uppercase ml-1">Mã số thuế *</label>
                               <input type="text" placeholder="MST" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold outline-none shadow-inner" value={formData.taxId} onChange={e => setFormData({...formData, taxId: e.target.value})} />
                             </div>
                             <div className="md:col-span-2 space-y-1">
                               <label className="text-[10px] font-black text-gray-400 uppercase ml-1">Địa chỉ trụ sở *</label>
                               <input type="text" placeholder="Địa chỉ đăng ký kinh doanh..." className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold outline-none shadow-inner" value={formData.companyAddress} onChange={e => setFormData({...formData, companyAddress: e.target.value})} />
                             </div>
                           </div>
                        </div>
                        ...
                        <div className="space-y-8 bg-emerald-50/40 p-8 rounded-[3rem] border border-emerald-50 shadow-inner">
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                             <div className="space-y-1">
                               ...
                               <input type="text" placeholder="Họ tên người liên hệ" className="w-full p-4 bg-white border-2 border-emerald-100 rounded-2xl focus:border-emerald-500 font-bold outline-none shadow-sm" value={formData.registrantName} onChange={e => setFormData({...formData, registrantName: e.target.value})} />
                             </div>
                             <div className="space-y-1">
                               <label className="text-[10px] font-black text-[#1B4332] uppercase ml-1 flex items-center gap-2"><Briefcase size={12}/> Chức vụ *</label>
                               <input type="text" placeholder="VD: Trưởng phòng Kinh doanh" className="w-full p-4 bg-white border-2 border-emerald-100 rounded-2xl focus:border-emerald-500 font-bold outline-none shadow-sm" value={formData.registrantPosition} onChange={e => setFormData({...formData, registrantPosition: e.target.value})} />
                             </div>
                             <div className="space-y-1">
                               ...
                               <input type="tel" placeholder="09xxx..." className="w-full p-4 bg-white border-2 border-emerald-100 rounded-2xl focus:border-emerald-500 font-bold outline-none shadow-sm" value={formData.registrantPhone} onChange={e => setFormData({...formData, registrantPhone: e.target.value})} />
                             </div>
                             <div className="space-y-1">
                               <label className="text-[10px] font-black text-[#1B4332] uppercase ml-1 flex items-center gap-2"><Mail size={12}/> Email liên hệ *</label>
                               <input type="email" placeholder="example@mail.com" className="w-full p-4 bg-white border-2 border-emerald-100 rounded-2xl focus:border-emerald-500 font-bold outline-none shadow-sm" value={formData.registrantEmail} onChange={e => setFormData({...formData, registrantEmail: e.target.value})} />
                             </div>
                           </div>
                        </div>
                      </div>
                          ...
                          {[
                            ...
                            <div key={item.key} className="p-6 border-2 border-gray-50 rounded-[2.5rem] bg-gray-50/30 flex justify-between items-center group hover:bg-white hover:border-emerald-200 transition-all shadow-sm">
                               <div className="flex items-center gap-4">
                                  ...
                                  <div><span className="font-black uppercase text-xs tracking-widest block">{item.label}</span>{formData.legalDocs[item.key].name && <span className="text-[10px] font-bold text-emerald-600 italic">Đã chọn: {formData.legalDocs[item.key].name}</span>}</div>
                               </div>
                               <label className="cursor-pointer px-6 py-3 bg-white border-2 border-gray-100 text-[10px] font-black uppercase rounded-2xl hover:bg-emerald-50 transition-all flex items-center gap-2 shadow-sm"><FileUp size={16}/> Tải file<input type="file" className="hidden" onChange={(e) => handleFileUpload(e, item.key)} /></label>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {step === 3 && (
                      <div className="space-y-8 animate-in fade-in text-left">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-emerald-50/20 p-10 rounded-[3.5rem] border border-emerald-50 shadow-inner">
                          ...
                          <div className="space-y-2">
                            <label className="text-[10px] font-black text-gray-400 uppercase ml-1">MOQ đơn hàng *</label>
                            <input type="text" placeholder="VD: 5 Triệu hoặc 20 Thùng" className="w-full p-4 bg-white border-2 border-emerald-100 rounded-2xl outline-none font-bold focus:border-emerald-500 shadow-sm" value={formData.moq} onChange={e => setFormData({...formData, moq: e.target.value})} />
                          </div>
                          <div className="space-y-2">
                    ...
                    {step === 4 && (
                      <div className="space-y-8 animate-in fade-in">
                        <div className="flex justify-between items-center border-b border-emerald-50 pb-6">
                          ...
                          <button type="button" onClick={addProduct} className="bg-[#1B4332] text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-black shadow-xl transition-all"><Plus size={18}/> Thêm hàng</button>
                        </div>
                          ...
                          {formData.products.map((p, idx) => (
                            <div key={p.id} className="p-10 border-2 border-emerald-50 rounded-[4rem] bg-white relative hover:shadow-2xl transition-all border-l-[20px] border-l-emerald-600 shadow-lg text-left">
                              <div className="flex justify-between items-center mb-10">
                                <span className="bg-[#1B4332] text-white text-[11px] font-black px-6 py-2.5 rounded-full uppercase italic tracking-tighter">Sản phẩm #{idx + 1}</span>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                ...
                                <div className="space-y-1">
                                   ...
                                   <select className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl outline-none font-bold text-sm shadow-inner" value={p.category} onChange={e => updateProduct(p.id, 'category', e.target.value)}>
                                ...
                                <div className="md:col-span-3 space-y-1 border-t border-gray-50 pt-6 mt-2">
                                   <label className="text-[10px] font-black text-gray-400 uppercase block mb-2">Hệ thống siêu thị đang phân phối (Winmart, Coop, Lotte...)</label>
                                   <input type="text" placeholder="Nhập tên các đối tác phân phối hiện tại (nếu có)" className="w-full p-5 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-emerald-500 font-bold text-sm outline-none shadow-inner" value={p.existingDistribution} onChange={e => updateProduct(p.id, 'existingDistribution', e.target.value)} />
                                </div>
                                <div className="md:col-span-3 grid grid-cols-2 gap-6 mt-4">
                                   <label className="w-full py-6 border-2 border-dashed border-emerald-100 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 transition-all group bg-gray-50/20 text-center text-gray-400">
                                      <FileUp size={24} className="text-emerald-300 group-hover:text-emerald-600 animate-pulse"/>
                                      <span className="text-[10px] font-black mt-2 uppercase">{p.complianceDoc || 'Hồ sơ công bố PDF'}</span>
                                   ...
                                   <label className="w-full py-6 border-2 border-dashed border-emerald-100 rounded-3xl flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 transition-all group bg-gray-50/20 text-center text-gray-400">
                                      ...
                                      <span className="text-[10px] font-black mt-2 uppercase">{p.productImage || 'Ảnh sản phẩm'}</span>
            ...
            {view === 'admin_dashboard' && isAdmin && (
              <div className="space-y-10 animate-in fade-in">
                 <div className="flex flex-col md:flex-row justify-between items-center gap-8 bg-white p-12 rounded-[4rem] border border-emerald-50 shadow-sm shadow-emerald-900/5">
                    <div>
                      <h2 className="text-4xl font-black text-[#1B4332] tracking-tight italic uppercase leading-none">Thẩm định MD<br/><span className="text-sm opacity-50 uppercase tracking-widest font-sans not-italic">Cloud Owner Console</span></h2>
                      <div className="flex items-center gap-3 mt-4"><span className="bg-[#1B4332] text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest italic shadow-lg shadow-emerald-900/20 underline decoration-emerald-500">Chủ sở hữu: {OWNER_EMAIL}</span></div>
                    </div>
                    <button onClick={() => setView('supplier_dashboard')} className="px-8 py-4 bg-emerald-50 text-emerald-800 rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-emerald-100 transition-all border-2 border-emerald-100">Thoát Quản Trị</button>
                 </div>

                 <div className="bg-white rounded-[4rem] border border-emerald-50 shadow-2xl overflow-hidden">
                    <table className="w-full text-left border-collapse">
                      <thead><tr className="bg-[#FDFBF7] text-gray-400 text-[10px] font-black uppercase tracking-[0.2em] border-b border-emerald-50"><th className="px-12 py-8">Nhà cung cấp / Đại diện</th><th className="px-12 py-8">Mặt hàng</th><th className="px-12 py-8 text-center">Trạng thái</th><th className="px-12 py-8 text-center">Xử lý MD</th></tr></thead>
                      <tbody className="divide-y divide-emerald-50">
                        {submissions.length === 0 ? (
                          <tr><td colSpan="4" className="p-32 text-center text-gray-300 font-black italic uppercase tracking-widest text-[10px] opacity-50">Chưa có hồ sơ mới</td></tr>
                        ) : (
                          submissions.map(sub => (
                            <tr key={sub.id} className="hover:bg-emerald-50/20 transition-all">
                              <td className="px-12 py-10">
                                ...
                                <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest italic">{sub.registrantName} ({sub.registrantPhone})</span>
                              </td>
                              <td className="px-12 py-10">
                                <div className="flex flex-wrap gap-2">{sub.products?.map((p, i) => <span key={i} className="text-[10px] font-bold text-emerald-700 italic border-b border-emerald-50">{p.name}</span>)}</div>
                              </td>
                              <td className="px-12 py-10 text-center"><span className="bg-emerald-100 text-emerald-600 px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-200">Mới</span></td>
                              <td className="px-12 py-10">
      ...
      <footer className="py-14 text-center text-[10px] font-black text-gray-300 uppercase tracking-[0.4em] border-t border-emerald-50 bg-white italic opacity-70">
        © 2024 FARMERS MARKET • CLOUD HUB v5.6.5 • SRM INFRASTRUCTURE • OWNER: {OWNER_EMAIL}
      </footer>
    </div>
  );
};
